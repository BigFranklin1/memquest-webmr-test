...
**重要：这不是“所有地方已经同步”的证明。** 写本文档时，上一轮“Scan 全屏取景”优化及相关测试还在本地未提交改动中；本文档也是新文件。接手者直接 clone GitHub，不会自动得到这些本地文件。原交接人需要先审核、提交并推送要交付的版本，再按需要发布 Vercel。
...
扫描时，相机占据整个视口；大面积说明、品牌栏和导航会暂时隐藏。仅保留退出按钮、透明取景框和边缘进度。结果出现后恢复操作界面。这是最近一次迭代的核心变化。
...
- 结果卡展示事件、图片、文本摘要、日期和匹配分数，可以听故事、进入对应时间线、关联 Samuel Adams 或再次扫描。
...
**不能宣称：** 能识别人脸、任何物体、中文手写、任意历史事件，或者把真实人物投影到空间中。当前 Scan 是英文文字 → 四个预设事件的匹配。
...
| Scan | 始终使用普通 getUserMedia 相机，便于 OCR 读取帧 | 本地英文文字识别 |
| Harbor | 支持时 immersive-vr；否则方向传感器 / 拖动 | 可交互 3D 场景，提供设备适配 |

手机能随着转动改变视角，不自动说明已经进入 WebXR；它也可能是设备方向传感器模式。Scan 成功也不说明实现了平面检测、空间锚点或物体追踪。
...
- [ ] 能说清三个边界：不是自由 AI 对话、不是实物空间锚定、不是云端学习账号。
