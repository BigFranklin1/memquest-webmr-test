...
扫描时，相机占据整个视口；大面积说明、品牌栏和导航会暂时隐藏。仅保留退出按钮、透明取景框和边缘进度。结果出现后恢复轻量操作界面，同时继续显示相机：中央透明 Three.js 场景展示导入的 Samuel Adams 人物模型，并循环播放说话手势，并用一次短暂粒子效果表现生成过程，周围悬浮事件资料、语音、时间线、人物和再次扫描按钮。默认是原有屏幕空间投影；Stamp Act 和 Boston Tea Party 结果均提供默认关闭的 `Page anchor` 测试开关。
...
- Stamp Act 和 Boston Tea Party 的 `Page anchor` 打开后才加载对应事件的参考集合，各含两张封面。保持一张完整封面在画面内时，人物随封面移动和转动；失去封面后模型隐藏，关掉开关即可回到默认投影。配置与重新编译方法见文末“多参考图维护”。
...
**不能宣称：** 能识别人脸、任意物体、中文手写、任意历史事件，或者已经实现持久的现实世界锚点。当前 Scan 是英文文字 → 四个预设事件的匹配；图片追踪支持 Stamp Act 和 Boston Tea Party 各两张已配置封面，一次只追踪一张，属于当前页面会话内的实验开关，不是平面检测或 WebXR Anchors。
...
手机能随着转动改变视角，不自动说明已经进入 WebXR；它也可能是设备方向传感器模式。Scan 成功本身不说明实现了空间定位；只有用户主动打开 Stamp Act 或 Boston Tea Party 的 `Page anchor` 才运行预设图片追踪（自动选图，一次一张），而且仍不等于平面检测、持久空间锚点或任意物体追踪。
...
- “AR 能锚定任意真实物体并永久保留”——当前仅有默认关闭的 Stamp Act / Boston Tea Party 预设封面图片追踪，没有平面检测、持久 WebXR 锚点或任意实物识别。
...
## 补充：港口视觉精修（本地版本，2026-09-05）

场景现已加强近景木板、箱体、灯笼、中景船舶索具和后排仓库细节。水面从较平的深色平面改为带细波纹和天空盒反射的材质。原有四个线索及历史内容不变。

如果让 Codex 继续改场景，请同时提供 `src/HarborScene.jsx` 和 `src/harborVisuals.js`。后者将静态模型按材质和空间区域合并绘制，但不合并水面、透明物体及点击判定代理；不要给新增装饰误加线索 ID。

![港口桌面视觉精修](images/harbor-refined-desktop.jpg)

![港口手机横屏排布](images/harbor-refined-landscape.jpg)

以上为真实 Three.js 场景在桌面浏览器中的截图，横屏尺寸模拟手机视口，不是真机拍摄。单一 1200×800 视角的绘制调用由 544 次降至约 295 次，不代表手机帧率提升相同比例。57 项应用测试和 4 项 Sites 测试通过；真实手机 WebXR、陀螺仪及持续运行温度仍需验证。此轮尚未发布到 GitHub / Vercel。

## 补充：手机水面优化（本地版本，2026-09-05）

这轮把多层透明反光条替换为一个轻量水面。近处可见缓慢、交错的细波纹，远处逐渐融入雾色。水面反射来自天空盒，灯光高光是简化表现，并非真实船舶或建筑倒影。

让 Codex 调整水面时，提供 `src/harborWater.js`；纹理生成在 `src/harborVisuals.js`，场景接入在 `src/HarborScene.jsx`。可以说：“让波纹更平缓，保留单次绘制和手机像素倍率限制，不添加实时镜面反射。”不要仅为了更锐利就取消手机画质预算。

![水面优化：模拟手机横屏](images/harbor-mobile-water.jpg)

上图来自桌面浏览器模拟 844×390、触屏和三倍屏幕像素密度，不是真机照片。验证确认采用 525 个水面顶点、1.25 渲染像素倍率，且没有着色器或运行时错误。桌面同一初始视角总绘制调用约从 295 降至 250；这不等于手机帧率提升比例。60 项应用测试、4 项站点测试、生产构建和三个视口下的 12 次线索交互检查通过。实际 iPhone / Android 帧率、持续运行发热与 XR 仍需真机检查。发布状态以 GitHub 最新提交与 Vercel 当前生产部署为准。

## 多参考图维护（2026-09-06）

现在 Stamp Act 有两张封面，Boston Tea Party 有两张（保留之前的 Perspectives，加上 Hourly History），都出现 Samuel Adams。用户只需先扫描英文标题，打开 Page anchor 后对准该事件任一完整封面，不需要点击缩略图选图。换图不改变历史事件；想换事件请 Scan again。未启用开关时不会启动图片追踪。

素材在 src/assets/tracking/，对应关系在 src/anchorTargets.js；其中 images 的先后顺序就是识别编号，修改顺序或增加图片后一定重新编译相应事件。请按 README“多参考图锚定”的命令操作。修改图片后还需生成提示用缩略图，再测试和构建；不要只改 PNG 就直接发布。

测试页 /tests/multi-target-browser-fixture.html 使用模拟相机输入运行真实 MindAR 算法；四张图片均已匹配正确索引。/tests/scan-camera-browser-fixture.html?visual=result&event=stamp-act&target=1 用注入姿态检查第二张图的结果界面；event=tea-party 可检查倾茶事件。这些测试不是实拍，真实纸张反光、弯曲、遮挡和手机帧率仍须用 iPhone / Android 验证，不能在作品集中称为“任意图片必定稳定锚定”。
